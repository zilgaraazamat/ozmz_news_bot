...
def create_game_template(name, field, address, default_time, price, max_players,
                          ...
                          num_teams=None, players_per_team=None):
    with _lock, _conn() as c:
        cur = c.execute("""INSERT INTO game_templates(
                name, field, address, default_time, price, max_players, duration,
                description, payment_link, image, created_by, num_teams, players_per_team,
                created_at, updated_at
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))""",
            (name, field, address, default_time, price, max_players, duration,
             description, payment_link, image, str(created_by), num_teams, players_per_team))
...
def update_game_template(template_id, name, field, address, default_time, price,
    ...
    with _lock, _conn() as c:
        c.execute("""UPDATE game_templates SET
                name=?, field=?, address=?, default_time=?, price=?, max_players=?, duration=?,
                description=?, payment_link=?, image=?, num_teams=?, players_per_team=?,
                updated_at=datetime('now')
            WHERE id=?""",
            (name, field, address, default_time, price, max_players, duration,
             description, payment_link, image, num_teams, players_per_team, template_id))

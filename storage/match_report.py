...
и не были и не должны быть тронуты этим модулем:
  ...
  - storage/teams_slots.py — кто в какой команде (get_team_members),
                              ...
                              этот модуль состав команд не меняет
...
def get_match_report(game_id, viewer_user_id=None):
    ...
    используется только чтобы посчитать его личный результат
    (victory/defeat/draw/None), больше ни на что не влияет.
    """
    ...
    # Считаем по каждой команде: сумму голов её игроков и число игроков
    # (включая безымянных гостей — у них user_id нет, поэтому голов у них
    # тоже не может быть, но на поле они были и должны попасть в счётчик
    # "игроков").
    teams_by_index = {}
    for m in members:
        idx = m["team_index"]
        ...
        if m["user_id"]:
            ...
            team["goals"] += goals_by_user.get(m["user_id"], 0)
